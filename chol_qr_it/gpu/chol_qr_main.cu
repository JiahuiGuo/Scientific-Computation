...
void chol_qr_it_GPU(integer m, integer n, real *d_A, integer lda, real *d_G, real *R, real *h_work, integer lwork);
...
main( int argc, char** argv) 
{
    ...
    /* Matrix size */
    integer N, M;                 // NxM would be the size of the matrices 
                              // (M columns) that we would orthogonalize
    real *d_A, *d_G;         // d_A is array for A on the device (GPU)
    real *h_work, *h_tau;    // work space and array tau on the host
    real *h_A, *h_Q1, *h_Q2; // These would be the same NxM matrices 
    real *h_R, *h_G;

    integer info[1], lwork, i;
    ...
    printf("LAPACK CPU Processing time: %f (ms) \n", cutGetTimerValue(timer));
